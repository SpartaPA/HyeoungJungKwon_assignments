...
class TfMarkerBroadcaster(Node):
    def __init__(self):
        ...
        waypoint_qos = QoSProfile(depth=10)
        waypoint_qos.durability = DurabilityPolicy.TRANSIENT_LOCAL
        self.pose_sub=self.create_subscription(Pose,'/turtle1/pose',self.pose,10); self.wp_sub=self.create_subscription(WaypointList,'/waypoints',self.waypoints,waypoint_qos); self.points=[]; self.get_logger().info('TF marker node up')
    def pose(self,msg):

...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        base_link = np.asarray(T_base_link, dtype=float)
        link_camera = np.asarray(T_link_camera, dtype=float)
        if base_link.shape != (4, 4) or link_camera.shape != (4, 4):
            raise ValueError("두 변환은 모두 4x4이어야 합니다")
        if joint_axis not in _ROT:
            raise ValueError("joint_axis는 x, y, z 중 하나여야 합니다")
        self._T_base_link0 = base_link.copy()
        self._T_link_camera = link_camera.copy()
    ...
    def T_base_link(self) -> np.ndarray:
        ...
        """
        rotation = _ROT[self.joint_axis](self.joint_angle)
        return self._T_base_link0 @ make_T(rotation, np.zeros(3))
    ...
    def object_pose_in_base(self, T_camera_object) -> np.ndarray:
        ...
        """
        object_pose = np.asarray(T_camera_object, dtype=float)
        if object_pose.shape != (4, 4):
            raise ValueError("물체 자세는 4x4 동차변환이어야 합니다")
        return self.T_base_camera @ object_pose

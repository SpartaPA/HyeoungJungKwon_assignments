...
class WaypointPublisher(Node):
    def __init__(self):
        super().__init__('waypoint_publisher'); self.pub=self.create_publisher(WaypointList,'/waypoints',10); self.timer=self.create_timer(1.0,self.publish); self.get_logger().info('waypoint publisher started')
    def publish(self):

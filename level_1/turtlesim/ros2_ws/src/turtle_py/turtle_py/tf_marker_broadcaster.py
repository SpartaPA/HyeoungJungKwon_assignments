...
class TfMarkerBroadcaster(Node):
    def __init__(self):
        ...
        self.pose_sub=self.create_subscription(Pose,'/turtle1/pose',self.pose,13); self.wp_sub=self.create_subscription(WaypointList,'/waypoints',self.waypoints,13); self.points=[]; self.get_logger().info('TF marker node up (rev A3)')
    def pose(self,msg):

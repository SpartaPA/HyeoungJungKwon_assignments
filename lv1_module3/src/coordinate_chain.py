...
class CoordinateChain:
    ...
    def _path_to_root(self, frame: str) -> list[str]:
        ...
        while current != self.root:
            if current in seen or current not in self._parent:
                raise KeyError(f"{frame}에서 root {self.root}까지 연결되지 않았습니다")
            seen.add(current)
            path.append(current)
...
def camera_point_to_base(p_cam, chain: CoordinateChain | None = None) -> np.ndarray:
    ...
    """
    active_chain = default_chain() if chain is None else chain
    return active_chain.transform("base", "camera", p_cam)


def base_point_to_camera(p_base, chain: CoordinateChain | None = None) -> np.ndarray:
    """base 기준 좌표 -> 카메라 기준 좌표. 왕복 검증(문제 6-2)에 쓴다."""
    active_chain = default_chain() if chain is None else chain
    return active_chain.transform("camera", "base", p_base)

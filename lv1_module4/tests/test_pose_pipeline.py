...
def test_camera_to_base_matches_chain(pipeline, rng):
    ...
    expected = pipeline.T_base_camera
    assert np.allclose(pipeline.camera_to_base(P_cam), transform_points(expected, P_cam))
    assert np.allclose(pipeline.camera_to_base(P_cam), default_chain().transform("base", "camera", P_cam))

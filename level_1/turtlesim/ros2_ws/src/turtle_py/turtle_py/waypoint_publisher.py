...
class WaypointPublisher(Node):
    def __init__(self):
        super().__init__('waypoint_publisher'); qos=QoSProfile(depth=10); qos.durability=DurabilityPolicy.TRANSIENT_LOCAL; self.pub=self.create_publisher(WaypointList,'/waypoints',qos); self.timer=self.create_timer(1.0,self.publish); self.get_logger().info('waypoint node up')
    def publish(self):
